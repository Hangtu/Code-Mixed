...
if ($db->num_rows($res) == 0) {
	...
	WHERE a.id_licitacion=".$arr['id_licitacion']." ORDER BY a.partida::FLOAT LIMIT $resultsByPage OFFSET 0";
	$res  = $db->query($querySQL);
	...
	//OBTENER PAGINAS
	$querySQL = "SELECT partida FROM vta_licitacion_detalle WHERE id_licitacion=".$arr['id_licitacion'];
	$res  = $db->query($querySQL);
	$noRes = $db->num_rows($res);
	//

	$arr['paginas'] = ceil(($noRes/$resultsByPage));
	if($noRes == 0){
		$arr['paginas'] = 1;
	}
	$arr['rows'] = $arrRows; 
	...
	$page = ($data->page * $resultsByPage);
	
	$querySQL = "SELECT a.*, b.nombre as nom_proveedor, c.laboratorio as nom_laboratorio FROM vta_licitacion_detalle a 
	LEFT JOIN cat_proveedor b ON a.proveedor = b.id_proveedor 
	LEFT JOIN cat_laboratorio c ON a.laboratorio = c.id_laboratorio
	WHERE a.id_licitacion=".$data->licitacion." ORDER BY a.partida::FLOAT LIMIT $resultsByPage OFFSET $page";
	$res  = $db->query($querySQL);
	...
	case 'search_by_partida':
	
	$querySQL = "SELECT a.*, b.nombre as nom_proveedor, c.laboratorio as nom_laboratorio FROM vta_licitacion_detalle a 
	LEFT JOIN cat_proveedor b ON a.proveedor = b.id_proveedor 
	LEFT JOIN cat_laboratorio c ON a.laboratorio = c.id_laboratorio
	WHERE a.id_licitacion=".$data->licitacion." AND (a.partida = '$data->partida' OR UPPER(a.clave_cbn) = UPPER('$data->partida')) ORDER BY a.partida::FLOAT";
	$res  = $db->query($querySQL);
	...
	foreach ($rows as $masterKey => $row) {
		foreach ($row as $key => $value) {
			$rows[$masterKey][$key] = utf8_decode(trim(preg_replace('/[^A-Za-z0-9 \s\s+.\-,&]ÁÉÍÓÚáéíóú\s/', '', $value))); //LIMPIA LOS DATOS ANTES DE INSERTARLOS AL ARRAY 
		}
	}
	...
	// if there is proveedor then we've  to validate the extras values : clave_sistema','costo','precio','laboratorio', 'codigo_barras','empresa'
		...
		foreach ($registro as $key => $value) {
			...
			if($key == 'proveedor' && $value != ''){
				...
				$proveedorSQL = "SELECT id_proveedor, nombre 
				FROM cat_proveedor 
				WHERE REPLACE(REPLACE(REPLACE(UPPER(nombre),'.',''),',',''),'SA DE CV','') LIKE REPLACE(REPLACE(REPLACE(UPPER('%$value%'),'.',''),',',''),'SA DE CV','') AND estatus = 1";
				$res = $db->query($proveedorSQL);
				$no_resp = $db->num_rows($res);

				if ($no_resp > 1) {
					echo "El nombre proveedor debe ser mas especifico, se encontraron dos o mas resultados similares \n";
					http_response_code(401); exit();
				}

				if ($no_resp == 0){
					echo "El nombre de proveedor no existe \n";
					http_response_code(401); exit();
				}
			...
			if( $key == 'precio' && (($value == '') || !is_numeric($value)|| $value <= 0 )){
			...
			if($key == 'laboratorio' && $value != ''){
				...
				$laboratorioSQL = "SELECT id_laboratorio, laboratorio FROM cat_laboratorio WHERE  REPLACE(REPLACE(REPLACE(upper(laboratorio),'.',''),',',''),'''','' )= upper('$value') AND estatus = 1";
				...
				if ($no_resp > 1) {
					echo "El laboratorio debe ser mas especifico, se encontraron dos o mas resultados similares \n";
					http_response_code(401); exit();
				}

				if ($no_resp == 0){
					echo "El nombre de laboratorio no existe \n";
					http_response_code(401); exit();
				}
			...
			if ($key == 'empresa' && (strtolower($value) == '') ){
	...
	$partidas = array_count_values(array_column($rows, 'partida')); // CUNETA SI HAY PARTIDAS REPETIDAS
							...
							if($codigo_barras == 0){// SI NO HAY CODIGO DE BARRAS VALIDAR QUE NO EXISTAN CLAVES DUPLICADAS
							 ...
							 // SI HAY CODIGO DE BARRAS VALIDAR QUE SI EXITEN CLAVES DUPLICADAS ESTAN TENGAN CODIGO DE BARRAS DIFERENTES
							if ($codigo_barras == 1) {
								foreach ($claves as $key => $repetido) {
									if ($repetido > 1) {

										$posiciones = array();
										for ($i=0; $i < sizeof($rows) ; $i++) { 
											if (in_array($key, $rows[$i]))
											{
												$posiciones[$i] = $rows[$i]['codigo_barras'];
											}
										}


										$cb_repeat = array_count_values($posiciones);

										foreach ($cb_repeat as $cod_barra => $value) {
											if ($value > 1) {
												echo 'El codigo de barras '.$cod_barra.' esta duplicado para la clave '.$key;
												http_response_code(400); exit();
											}
										}
									}
	...
	foreach($rows AS $masterkey => $row){ // SE RECORRE EL ARREGLO DE TODOS LOS REGISTROS
		...
		//ELEMENTOS PARA HACER UPDATE
		if($rows[$masterkey]['newest'] == 0){
		foreach ($row as $key => $value) { // SE RECORREN LOS VALORES DENTRO DE ESE REGISTRO PARA LIMPIARLOS Y CONCATENARLOS
			$value = str_replace(array("\r\n", "\r", "\n"), " ", $value);
			$value = !empty($value) ? "'$value'" : "NULL";
			if ($key != "newest"){ // NO AGREGAR ESTA OPCION
				$sqlUpdate = "UPDATE vta_licitacion_detalle SET  $key = $value WHERE id_licitacion  = $id_licitacion AND partida ='".$rows[$masterkey]['partida']."'"; 
				$db->query($sqlUpdate);	
			}
		}
	}
							...
							foreach ($cellIterator as $cell) {  //OBTIENE LOS VALORES DE ESE RENGLON Y LOS ALMACENA EN EL ARREGLO
								...
								if($titulos == 0){ // VALIDACION DE LAS CABECERAS
									...
									for ($i=0; $i < sizeof($columnNames) ; $i++){ //OBTIENE LOS VALORES DE CADA COLUMNA
										$data[$columnNames[$i]] = utf8_decode(trim(preg_replace('/[^A-Za-z0-9 \s\s+.\-,&]ÁÉÍÓÚáéíóú\s/', '', $datos[$i]))); //LIMPIA LOS DATOS ANTES DE INSERTARLOS AL ARRAY 
									}
								...
								foreach ($claves as $key => $repetido) {
									if ($repetido > 1) {
										...
										for ($i=0; $i < sizeof($allData) ; $i++) { 
											if (in_array($key, $allData[$i]))
											{
												$posiciones[$i] = $allData[$i]['codigo_barras'];
											}
										}
